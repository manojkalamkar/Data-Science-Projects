...
Where as Decision Tree(CART) model is able to predict correctly 65.35% customers to be churned as true but also predicts incorrectly for some 2.68% customers as churned(which also is not true). So it depends upon Business which model it would like to use if it would like to go for higher predictive power at cost of some Fall Out ratio/error deviation Decision Tree is best. ELSE if company wants to stress more on minimizing error rate than RF can be good one.
